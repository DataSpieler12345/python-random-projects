...
# Scenario
# Your task is to write a simple program that simulates playing tic-tac-toe with the user. To make it easier, we have decided to simplify the game. Here are our rules:

# 1. the machine (i.e., the program) will play using the 'X's;

# 2. the user (i.e. you) will play using the 'O's;

# 3. the first move is the machine's - it always places an 'X' in the center of the board;

# 4. all the squares are numbered starting with 1 (see example for reference)

# 5. the user enters his move by entering the number of the chosen square - the number must be valid, i.e. an integer value greater than 0 and less than 10, and it cannot be a square that is already occupied;

# 6. the program checks if the game is over - there are four possible verdicts: the game continues, the game ends in a draw, you win, or the machine wins;

# 7. the machine responds with its movement and the state of the game is verified;

# 8. no artificial intelligence should be implemented - the machine will choose a square randomly, that is enough for this game.
